...
function calculateScores(reviews: Review[]): Score[] {
  ...
  return attributes.map(({ label, keywords, issueTypes }) => {
    ...
    return {
      label,
      value: Math.max(1, Math.min(100, Math.round(avgScore * 100))),
      max: 100,
    };
  });
}
...
export default function DashboardPage() {
    ...
    async function init() {
      try {
        const [prodData, reviewData] = await Promise.all([
          fetchProductsAction(),
          fetchLatestReviewsAction(10000),
        ]);
  ...
  const handleSend = async (userInput: string) => {
    ...
    try {
      ...
      if (
        aiResponse.matchedReviewIds &&
        aiResponse.matchedReviewIds.length > 0
      ) {
        ...
        filtered = await fetchReviewsByKeywordsAction(
          aiResponse.keywords,
          10000,
        );
      }
  ...
  const handleExportExcel = async (msg: Message) => {
    try {
      ...
      link.setAttribute(
        "download",
        `tones_reviews_${new Date().getTime()}.csv`,
      );
  ...
  const handlePadSelect = async (product: Product) => {
    setIsLoading(true);
    try {
      const productReviews = await fetchReviewsByProductAction(
        product.id,
        10000,
      );
      ...
      setScores(
        productReviews.length > 0
          ? calculateScores(productReviews)
          : DEFAULT_SCORES,
      );
    } catch (err: any) {
